...
async function injectButtons(container: HTMLElement, playableItem: UserItem) {
  ...
  group.className =
    "btnExternalPlayerGroup verticalFieldItem detailButtons flex align-items-flex-start flex-wrap-wrap focuscontainer-x detail-lineItem detailButtons-margin focusable";
...
`;
    ...
    btn.onclick = () => {
      ...
      const mediaSourceId = mediaSourceSelectValue || Id;
      ...
      const allSubtitles = currentMediaSource?.MediaStreams?.reduce(
        (acc, item) => {
          if (item.IsExternal === true && item.SupportsExternalStream === true && item.Type === "Subtitle") {
            const url =
              `${globalThis.location.origin}/Videos/${mediaSourceId}/Subtitles/${item.Index}/Stream.${item.Codec}?Token=${globalThis.ApiClient._serverInfo.AccessToken}&UserId=${globalThis.ApiClient._serverInfo.UserId}`;
            acc.push({
              url,
      ...
      const title = [SeriesName, SeasonName, Name, titleIndex].filter(Boolean).join(" ") ||
        "Video";
      const videoUrl =
        `${globalThis.location.origin}/Videos/${Id}/stream?MediaSourceId=${mediaSourceId}&Static=true&FakeDirectStream=true&Token=${globalThis.ApiClient._serverInfo.AccessToken}&UserId=${globalThis.ApiClient._serverInfo.UserId}`;
  ...
  const mainDetailButtons = container.querySelector(".detailSectionContent");
  if (mainDetailButtons) {
    mainDetailButtons.before(group);
  }
}

globalThis.addEventListener("viewshow", async function (e) {
  console.log("[njzy debug]", e);
  if (globalThis.location.hash.includes("/details")) {
    ...
    const container = detailPageElement?.querySelector(".detailSection") as HTMLElement;
    if (container) {
